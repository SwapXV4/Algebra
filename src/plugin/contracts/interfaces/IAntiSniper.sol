...
/// @title The interface for the AntiSniper
...
interface IAntiSniper is IVolatilityOracle, IDynamicFeeManager, IFarmingPlugin {
  ...
  /// @notice Emitted when the plugin is constructed
  /// @param _pool plugin's associated pool
  /// @param _factory algebra factory
  /// @param _pluginFactory plugin factory
  event Constructed(address _pool, address _factory, address _pluginFactory);
  ...
  /// @notice Returns whether the plugin was constructed
  function isConstructed() external view returns (bool);

  /// @notice Constructs the plugin
  /// @dev This is meant to be called once by the factory
  function construct(address _pool, address _factory, address _pluginFactory) external;
  ...
  /// @notice Get the current auxiliary fee configuration
